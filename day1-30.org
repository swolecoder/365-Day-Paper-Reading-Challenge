...
30. [[#day-30-ai][Day 30: Graph Few-shot Learning via Knowledge Transfer]]
...
*** Problem
Most GNNs have shallow layers with a limited receptive field and may not achieve satisfactory performance especially when the number of labeled nodes is quite small.

*** Method
They proposed a graph few-shot learning (GFL) algorithm that incorporates prior knowledge learned from auxiliary graphs to improve classification accuracy on the target graph.

*** Result
Extensive experiments and ablation studies on four real-world graph datasets demonstrate the effectiveness of their proposed model and the contribution of each component.
...
30. Yao, H., Zhang, C., Wei, Y., Jiang, M., Wang, S., Huang, J., ... & Li, Z. (2019). Graph few-shot learning via knowledge transfer. arXiv preprint arXiv:1910.03053.